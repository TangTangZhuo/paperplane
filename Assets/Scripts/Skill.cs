...
public class Skill : MonoBehaviour {
	...
	public void OnPowerBtn(){
		...
		power_Price_text.text = (power_price).ToString ();
		UpdateButton ();
	}

	public void OnFlipBth(){
		gold_C.UseGold (flip_price);
		gold -= flip_price;
		PlayerPrefs.SetInt ("gold", gold);
		flip_level++;
		...
		flip_price_text.text = (flip_price).ToString ();
		UpdateButton ();
	}

	public void OnOfflineBtn(){
		gold_C.UseGold (offline_price);
		gold -= offline_price;
		PlayerPrefs.SetInt ("gold", gold);
		offline_level++;
		...
		offline_price_text.text = (offline_price).ToString ();
		UpdateButton ();
	}

	void UpdateText(){
		flip_price = PlayerPrefs.GetInt ("flipPrice", 281);
		offline_price = PlayerPrefs.GetInt ("offlinePrice", 281);
		power_price = PlayerPrefs.GetInt ("powerPrice", 281);
		flip_level = PlayerPrefs.GetInt ("flipLevel", 1);
		offline_level = PlayerPrefs.GetInt ("offlineLevel", 1);
		power_level = PlayerPrefs.GetInt ("powerLevel", 1);
		flip_Level_text.text = flip_level.ToString ();
		flip_price_text.text = flip_price.ToString ();
		offline_Level_text.text = offline_level.ToString ();
		offline_price_text.text = offline_price.ToString ();
